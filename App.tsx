...
const relativeSin = (yPosition: number) => Math.sin((yPosition - 300) * (Math.PI / 360));
...
const ConfettiPiece = ({ startingXOffset, startingYOffset }: Offset) => {
  ...
  const matrix = useComputedValue(() => {
    ...
    const mat3 = toMatrix3(processTransform3d([{ rotateY: rotateY }, { rotateX: rotateX }, { rotateZ: rotateZ }]));
  ...
  return (
    <Group matrix={matrix} origin={origin}>
      <RoundedRect r={8} x={startingXOffset} y={yPosition} height={WIDTH} width={HEIGHT} />
    </Group>
  );
};
  ...
  return (
    <View style={styles.container}>
      ...
      <Pressable onPress={startAnimation} style={styles.button}>
        <Text style={styles.buttonText}>بمال!</Text>
      </Pressable>
    </View>
  );
};
